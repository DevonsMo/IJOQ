...
# Import is successful
    ...
    min_python_version_list = min_python_version.split(".")
    current_python_version_list = current_python_version.split(".")
    if current_python_version_list[0] == min_python_version_list[0] \
            and current_python_version[1] >= min_python_version_list[1]:

        print("You appear to be using an unsupported version of Python! "
              "Try to install the most updated version of Python before running this program!")
        input()
        quit()
    ...
    class CalNoiseCalculationThread(Thread):
        ...
        def run(self):
            ...
            blur_number = int(cal_confirmed_blur_radius / 2)
    ...
    def cal_go_to_calibration():
        ...
        if cal_settings_options_tabs.index("current") == 0:
            ...
            if average_cell_count == 0:
                ...
                if average_cell_count <= 50:
                    cal_confirmed_blur_radius = round(50 / average_cell_count)
                else:
                    cal_confirmed_blur_radius = round(25 / average_cell_count)

                if cal_confirmed_blur_radius % 2 != 0:
                    cal_confirmed_blur_radius += 1
                cal_confirmed_blur_radius = min(cal_confirmed_blur_radius, 10)
    ...
    def cal_run_calibration():
        ...
        for file_number in range(len(cal_confirmed_files)):
            ...
            try:
                ...
                blur_radius_values = [0, 2, 4, 6, 8, 10]
        ...
        for i in range(len(cal_confirmed_files)):
            ...
            for j in range(len(cal_blurred_images[i])):
                ...
                image_array = numpy.asarray(cal_blurred_images[i][j].convert("HSV"))
        ...
        cal_calibration_textbox.see(tkinter.END)
        current_blur_number = int(cal_confirmed_blur_radius / 2)
        for i in range(len(cal_confirmed_files)):
            image_name = cal_confirmed_file_names[i]
            ...
            image_array = numpy.asarray(cal_blurred_images[i][current_blur_number].convert("HSV"))
    ...
    def cal_results_blur_spinbox_update(_):
        sanitize_input(cal_results_blur_radius, True, "int", 0, 10)
    ...
    cal_settings_image_compression_spinbox.bind(
        "<FocusOut>",
        lambda e: sanitize_input(cal_image_compression, False, "int", 0, 1024))
    cal_settings_image_compression_spinbox.bind(
        "<Return>",
        lambda e: root.focus())
    cal_settings_blur_radius_spinbox = ttk.Spinbox(
        cal_settings_advanced_frame, from_=0, to=10, increment=2,
        wrap=True, textvariable=cal_blur_radius, width=6,
        validate="key", validatecommand=vcmd_int)
    cal_settings_blur_radius_spinbox.bind(
        "<FocusOut>",
        lambda e: sanitize_input(cal_blur_radius, True, "int", 0, 10))
    cal_settings_blur_radius_spinbox.bind(
        "<Return>",
        lambda e: root.focus())
    ...
    cal_results_settings_blur_slider = ttk.Scale(
        ...
        command=lambda e: cal_results_blur_radius.set(
            str(int(round(float(cal_results_blur_radius.get()) / 2) * 2))))
