...
# Import is successful
    ...
    class CalNoiseCalculationThread(Thread):
        def __init__(self):
            ...
            self.stop = False
            self.current_file = 0
            self.current_noise_filter = 0
            self.update_order = []

        def run(self):
            global cal_blurred_images, cal_brightness_maps, cal_processed_files

            # Save current image number and noise threshold
            self.current_file = cal_current_viewed_picture
            self.current_noise_filter = cal_confirmed_noise_filter
            ...
            blurred_image = cal_blurred_images[self.current_file]
            brightness_map = cal_brightness_maps[self.current_file]
